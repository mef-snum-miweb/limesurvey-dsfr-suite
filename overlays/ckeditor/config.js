...
/* =====================================================================
 * OVERLAY DSFR — additions à la config CKEditor de LimeSurvey.
 *
 * Ce fichier = config.js de l'image martialblog/limesurvey 6.16.16-260408
 * (copie fidèle ci-dessus) + le bloc ci-dessous. À RE-SYNCHRONISER si le
 * tag de l'image LimeSurvey change (cf. docker-compose.yml).
 *
 * Principe : on DÉCORE CKEDITOR.editorConfig (on exécute la config LS
 * d'origine, puis on ajoute nos éléments) — aucune logique LS n'est
 * réécrite. Portée : instance de démo (lab). Cf. ADR-071.
 * ===================================================================== */
(function () {
    var _lsEditorConfig = CKEDITOR.editorConfig;
    CKEDITOR.editorConfig = function (config) {
        if (typeof _lsEditorConfig === 'function') {
            _lsEditorConfig.call(this, config);
        }

        var base = CKEDITOR.basePath; // .../assets/packages/ckeditor/

        // Plugin natif "templates" (déjà embarqué dans l'image) : palette de
        // composants DSFR prêts à insérer.
        ...
        // Menu "Styles" : applique des CLASSES DSFR (pas des styles inline) —
        // ces classes survivent au sanitizer du thème.
        config.stylesSet = 'dsfr:' + base + 'dsfr-styles.js';

        // CSS DSFR dans l'iframe de l'éditeur : le WYSIWYG ressemble au rendu.
        config.contentsCss = (config.contentsCss ? [].concat(config.contentsCss) : [])
            .concat([base + 'dsfr-contents.css']);

        // Boutons dans les barres d'outils. LimeSurvey pilote l'éditeur inline
        // via config.toolbar = 'inline2' (référence à toolbar_inline2), et bascule
        // avec lsswitchtoolbars entre toolbar_inline / toolbar_inline2. On ajoute
        // donc le groupe DSFR à toutes les barres réellement définies (arrays).
