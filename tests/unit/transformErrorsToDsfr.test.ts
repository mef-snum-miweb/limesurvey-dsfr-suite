...
// --- Reproduire la logique depuis custom.js (lines 591-701, simplifiée) ---
// Note : attachErrorRemovalListeners est stubée car testée indirectement via E2E

function attachErrorRemovalListeners(
  _question: Element,
  _inputGroup: Element,
  _messagesGroup: Element
): void {
  // Stub — les listeners de correction sont testés en E2E
}

function transformErrorsToDsfr(): void {
  ...
  errorQuestions.forEach(function (question) {
    ...
    if (question.classList.contains('multiple-short-txt')) return;
    if (question.className.match(/array-/)) return;
    ...
    // Vérifier si un message existe déjà pour éviter la duplication
    ...
    // Trouver le message d'erreur LimeSurvey
    let lsErrorContainer: Element | null = null;
    let errorText = '';

    const inputElement = question.querySelector('.fr-input, input, textarea, select') as HTMLInputElement | null;
    ...
    if (isEmpty && mandatoryError) {
      ...
      for (let i = 0; i < validationErrors.length; i++) {
        const error = validationErrors[i] as HTMLElement;
    ...
    if (questionValidContainer) {
      (questionValidContainer as HTMLElement).style.display = 'none';
    }
