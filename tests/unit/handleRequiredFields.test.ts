...
// --- Reproduire la logique depuis custom.js (lines 343-570, simplifiée) ---

function handleRequiredFields(): void {
  const requiredFields = document.querySelectorAll(
    'input[required], textarea[required], select[required], ' +
    'input[aria-required="true"], textarea[aria-required="true"], select[aria-required="true"]'
  );

  const mandatoryQuestions = document.querySelectorAll(
    '.mandatory.question-container, .mandatory[id^="question"]'
  );

  if (requiredFields.length === 0 && mandatoryQuestions.length === 0) return;

  // Traiter les champs avec attribut required
  requiredFields.forEach((field) => {
    ...
    if (!label) {
      ...
      if (inputGroup) label = inputGroup.querySelector('.fr-label, .fr-fieldset__legend');
    }
    if (!label) {
      const fieldset = field.closest('fieldset');
      if (fieldset) label = fieldset.querySelector('.fr-fieldset__legend');
    }
    if (!label && field.getAttribute('aria-labelledby')) {
      label = document.getElementById(field.getAttribute('aria-labelledby')!);
    }
  ...
  mandatoryQuestions.forEach((question) => {
    ...
    inputs.forEach((input) => {
      ...
      if (el.type !== 'hidden' && !el.disabled && !el.hasAttribute('aria-required')) {
  ...
  // Notice (simplifié)
  if (document.getElementById('required-fields-notice')) return;

  const noticeHTML = '<p class="fr-text--sm">Les champs marqués d\'un * sont obligatoires</p>';
  const firstQuestion = document.querySelector('[id^="question"], .question-container');
  if (firstQuestion && firstQuestion.parentElement) {
